...
/**
 * ZcaMT main client class.
 *
 * ZcaMT is an unofficial Zalo API for personal accounts, developed from the
 * open-source {@link https://github.com/RFS-ADRENO/zca-js | zca-js} project.
 *
 * This class is functionally identical to the original `Zalo` class from
 * `zca-js` (kept below as `Zalo` for backward compatibility / migration).
 *
...
/**
 * `ZcaMT` is the preferred entry point for this package.
 *
 * ```ts
 ...
 * ```
 *
 * It extends the original `Zalo` class from `zca-js` without behavioral
 * changes, so every method, option, and event documented for `Zalo` also
 * applies to `ZcaMT`.
 */
export class ZcaMT extends Zalo {}

/**
 * Backward-compatible alias so code written against `zca-js` (`import { Zalo } from "zca-js"`)
 * keeps working after switching the package to `zca-mt` with a minimal diff.
 * Prefer importing `ZcaMT` in new code.
 */
export { ZcaMT as ZaloClient };
