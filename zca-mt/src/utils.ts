...
/**
 * Field names that must never appear unredacted in logs. Matched
 * case-insensitively against object keys.
 */
const SENSITIVE_LOG_FIELDS = ["cookie", "token", "imei", "secret", "authorization", "session"];
...
function isSensitiveKey(key: string): boolean {
    ...
    return SENSITIVE_LOG_FIELDS.some((field) => lower.includes(field));
}

/**
 * Deep-clones a value for logging while masking any object property whose
 * key matches a sensitive field name (cookie, token, imei, secret,
 * authorization, session — case-insensitive, substring match so e.g.
 * "secretKey" and "sessionId" are also caught).
 *
 * This is a best-effort safeguard for console output; it does not replace
 * being careful about what gets passed to the logger in the first place.
 */
export function redactSensitive(value: unknown, depth = 0): unknown {
    ...
    if (value && typeof value === "object") {
        // Avoid trying to redact exotic objects (e.g. CookieJar instances,
        // streams) by property-copying — just mask them outright since their
        // name alone (constructor) may hint at sensitive content.
        const ctorName = (value as { constructor?: { name?: string } }).constructor?.name;
        if (ctorName && /cookie|jar/i.test(ctorName)) return REDACTED;
...
export const logger = (ctx: { options: { logging?: boolean } }) => ({
    verbose: (...args: unknown[]) => {
        if (ctx.options.logging) console.log("\x1b[35m🚀 VERBOSE\x1b[0m", ...redactArgs(args));
    },
    info: (...args: unknown[]) => {
        if (ctx.options.logging) console.log("\x1b[34mINFO\x1b[0m", ...redactArgs(args));
    },
    warn: (...args: unknown[]) => {
        if (ctx.options.logging) console.log("\x1b[33mWARN\x1b[0m", ...redactArgs(args));
    },
    error: (...args: unknown[]) => {
        if (ctx.options.logging) console.log("\x1b[31mERROR\x1b[0m", ...redactArgs(args));
    },
    success: (...args: unknown[]) => {
        if (ctx.options.logging) console.log("\x1b[32mSUCCESS\x1b[0m", ...redactArgs(args));
    },
    timestamp: (...args: unknown[]) => {
        const now = new Date().toISOString();
        if (ctx.options.logging) console.log(`\x1b[90m[${now}]\x1b[0m`, ...redactArgs(args));
    },
});
